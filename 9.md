...
通过下面这个例子，我们来看看这两种编程范式在python中是如何统一的。
...
程序依旧可以如期运行。其实不论self还是x，类中的方法都默认会把实例作为方法的第一个参数传入。所以调用`a1.add(a2)`相当于`A.add(a1, a2)`。当然，在类中请使用self，而不是x，这是一个默认约定，为了提高代码的可阅读性。
