...
- 首先，外层作用域是看不见内层作用域的变量的，比如在global作用域是无法引用函数h和g的
- 其次，内层作用域中如果出现了不是该层的变量，他会依次从在外层作用域中找该变量，以此作为该变量的引用。比如函数h中的变量x，他的引用对象是g中的参数x。因此闭包是这类情形的一个特列。除此以外，还要注意下面两类情形的区别：
...
- 第二种情形：f作用域里有x，y，因此调用f，f不会去global作用域里找变量x，从而报错：x在赋值前未初始化。要解决这类问题，最简单的办法就是强行把f作用域里的x提升到global作用域：
